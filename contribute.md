...
- `task_params`
- `profiler_task_prompts.json` 裡的 `test_plan_task_question`
- `task_rubrics`
- `task_weights`
...
並在 `task_system_prompts.json` 補 runtime system prompt，在 `profiler_task_prompts.json` 的 `test_plan_task_question` 補一個可重複測試的代表題目。
